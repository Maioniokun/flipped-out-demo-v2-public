			...
			else
			{
					...
					&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[curSection].lengthInSteps))
				{
					...
					if (FlxG.mouse.justPressedRight)
						addNote(getStrumTime(dummyArrow.y) + sectionStartTime(), Math.floor(FlxG.mouse.x / GRID_SIZE), null, specialNote = true);
					else
						addNote(getStrumTime(dummyArrow.y) + sectionStartTime(), Math.floor(FlxG.mouse.x / GRID_SIZE), null, specialNote = false);
...
// || FlxG.keys.justPressed.X  || FlxG.keys.justPressed.C || FlxG.keys.justPressed.V
		...
		for (i in section)
			{
				...
				var note:Note = new Note(daStrumTime, daNoteInfo % 4, true, null, null, i[3]);
	...
	private function addNote(_noteStrum:Float, _noteData:Int, ?skipSectionCheck:Bool = false, specialNote:Bool = false):Void
	{
		var noteAdjust:Array<Int> = [0, 1, 2, 3, 4, 5, 6, 7];

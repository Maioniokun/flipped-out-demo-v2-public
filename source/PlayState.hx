...
class PlayState extends MusicBeatState
{
	...
	public static var campSongs:Array<String>;
	public static var bloodSongs:Array<String>;
	...
	public static var dialogue:Array<String> = [':bf:strange code', ':dad:>:]'];
	...
	override public function create()
	{
		...
		campSongs = ["tutorial", "triggered"];
		bloodSongs = ["slaughter"];
		...
		if (SONG.stage == null) {
			...
			if(campSongs.contains(SONG.song.toLowerCase())) { stageCheck = 'camp'; }
			else if(bloodSongs.contains(SONG.song.toLowerCase())) { stageCheck = 'blood'; }
		...
		if (stageCheck == 'camp')
		{
			defaultCamZoom = 0.65;
			curStage = 'camp';
			var sky:FlxSprite = new FlxSprite(-600, -225).loadGraphic('assets/images/flippy/triggered/sky.png');
			sky.antialiasing = true;
			sky.scrollFactor.set(0.5, 0.5);
			sky.active = false;
			add(sky);

			var backTrees:FlxSprite = new FlxSprite(-625, -200).loadGraphic('assets/images/flippy/triggered/backtrees.png');
			...
			var trees:FlxSprite = new FlxSprite(-625, -225).loadGraphic('assets/images/flippy/triggered/trees.png');
			trees.updateHitbox();
			...
			var ground:FlxSprite = new FlxSprite(-700, -150).loadGraphic('assets/images/flippy/triggered/ground.png');
			ground.updateHitbox();
			...
			boombox.frames = Paths.getSparrowAtlas("flippy/speaker");
			...
			campfire.frames = Paths.getSparrowAtlas("flippy/fire");
			campfire.animation.addByPrefix('idle', 'fire idle', 12, true);
			campfire.alpha = 0.8;
			campfire.animation.play('idle');
			add(campfire);
	
			table = new FlxSprite(-850, -200).loadGraphic('assets/images/flippy/triggered/table.png');
		...
		else if (stageCheck == 'blood')
		{
			defaultCamZoom = 0.65;
			curStage = 'blood';
			var sky:FlxSprite = new FlxSprite(-600, -225).loadGraphic('assets/images/flippy/slaughter/sky.png');
			sky.antialiasing = true;
			sky.scrollFactor.set(0.5, 0.5);
			sky.active = false;
			add(sky);

			var backTrees:FlxSprite = new FlxSprite(-625, -200).loadGraphic('assets/images/flippy/slaughter/backtrees.png');
			backTrees.updateHitbox();
			backTrees.antialiasing = true;
			backTrees.scrollFactor.set(0.7, 0.7);
			backTrees.active = false;
			...
			var trees:FlxSprite = new FlxSprite(-625, -225).loadGraphic('assets/images/flippy/slaughter/trees.png');
			trees.updateHitbox();
			trees.antialiasing = true;
			trees.scrollFactor.set(0.8, 0.8);
			trees.active = false;
			...
			var ground:FlxSprite = new FlxSprite(-700, -150).loadGraphic('assets/images/flippy/slaughter/ground.png');
			ground.updateHitbox();
			ground.antialiasing = true;
			ground.active = false;
			add(ground);
			...
			table = new FlxSprite(-850, -225).loadGraphic('assets/images/flippy/slaughter/table.png');
			table.setGraphicSize(Std.int(table.width * 1.1));
			table.updateHitbox();
			table.antialiasing = true;
			table.scrollFactor.set(1.3, 1.3);
		...
		if (curStage == 'camp' || curStage == 'blood')
			add(table);
		...
		healthBarBG2 = new FlxSprite(0, 0).loadGraphic('assets/images/healthBar2.png');
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, false, oldNote, null, noteSpec);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				case 'blood':
					babyArrow.frames = FlxAtlasFrames.fromSparrow('assets/images/BNOTE_assets.png', 'assets/images/BNOTE_assets.xml');
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if(daNote.tooLate){
					...
					if (daNote.alpha > 0.3){
						...
						if (daNote.camoNote == true)
							{
								noteMiss(daNote.noteData, 0.2, false, true);
								daNote.alpha = 0.3;
								vocals.volume = 0;
							}
						else if (daNote.nadeNote == true || daNote.burnNote == true)
							{
								trace("Good Miss!");
							}
						else
	...
	function goodNoteHit(note:Note):Void
	{
		...
		else if (!note.wasGoodHit)
		{
			...
			if (note.noteData >= 0){
				if (note.nadeNote == true)
					{
						health -= 0.5;
						trace("BAD HIT");
						FlxG.sound.play('assets/sounds/boom.ogg');
					}
				else if (note.burnNote == true)
					{
						FlxG.sound.play('assets/sounds/burnSound.ogg');
						new FlxTimer().start(0.01, function(tmr:FlxTimer) {health -= 0.0025; }, 200);
					}
				else
					{
	...
	function camFocusBF(){
		...
		switch (curStage)
		{
			case 'camp':
				...
				followY = boyfriend.getMidpoint().y + 175;
			case 'blood':
				followX = boyfriend.getMidpoint().x - 400;
				followY = boyfriend.getMidpoint().y + 175;
		}
		...
		if (SONG.song.toLowerCase() == 'tutorial')
			{
				FlxTween.tween(FlxG.camera, {zoom: 1.2}, (Conductor.stepCrochet * 4 / 1000), {ease: FlxEase.elasticInOut});
				followX = boyfriend.getMidpoint().x + 40;
				followY = boyfriend.getMidpoint().y - 75;
				new FlxTimer().start(4, function(tmr:FlxTimer) 
				{
					remove(dad);
					dad = new Character(100, 100, 'flippy-cool');
					add(dad);
					dad.y += 300;
					dad.x -= 100;
				});
